  ...
  useEffect(() => {
    const fetchReservationsByUser = async () => {
      const response = await axios.get(
        `http://${process.env.REACT_APP_API_URL}/reservation`
      );
      if (response.data) {
        ...
        response.data.forEach((dataBlock) => {
          ...
          if (dataBlock.type.split("_")[1] === "A") {
            reservationData.sport = reservationData.sport.concat(
              " ",
              "cancha 1"
            );
          ...
          if (dataBlock.user.name) {
            reservationData.identityNumber = String(
              dataBlock.user.identityNumber
            );
